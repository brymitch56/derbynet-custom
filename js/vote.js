...
function click_one_award(div) {
  ...
  g_awardid = award_id;
  console.log('Award clicked:', award_id, 'Is design award:', g_is_design_award[award_id]);
  ...
  if (g_is_design_award && g_is_design_award[award_id]) {
    console.log('Fetching eligible racers for design award:', award_id);
    $.ajax('action.php', {
      type: 'GET',
      data: {
      ...
      success: function(data) {
        console.log('Eligible racers response:', data);
        ...
        console.log('Number of eligible racers:', eligible_racers.length);
        eligible_racers.forEach(function(racer) {
          $("#racers div.ballot_racer[data-racerid=" + racer.racerid + "]").removeClass('hidden');
        });
      },
      error: function(xhr, status, error) {
        console.error('Error fetching eligible racers:', status, error);
      }
    });
  } else {
